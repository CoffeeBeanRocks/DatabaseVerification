...
# @Var csvError: holds a string of any warnings generated by pd.read_csv()
...
def sendSuccessEmail(recoverableErrors: str, df: pd.DataFrame):
    # Connecting to outlook
    olApp = win32com.client.Dispatch("Outlook.Application")
    olNS = olApp.GetNamespace("MAPI")
    ...
    if len(recoverableErrors) > 0:
        mailItem.Subject = 'Automatic Task Success (With Warnings)'
        mailItem.Body = "This is an automated email informing you that the task 'Default_TEST " \
                        "Upload' has been completed successfully but with the following " \
                        "warning:\n\n{}\n\n".format(recoverableErrors)
    else:
    ...
    if item is None:
        ...
        if '.csv' in str(Data.csvPath):
            ...
            try:
                ...
                # Records location of any skipped recoverableErrors in .csv
                ...
                raise Exception('Encountered a fatal error while reading file: {}'.format(fatal))
...
def updateAccess(filePath):
    ...
    # Saving updates
    connection.commit()
    sendSuccessEmail(Data.csvError, df2)
